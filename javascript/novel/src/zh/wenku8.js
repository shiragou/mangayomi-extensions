...
class DefaultExtension extends MProvider {
    ...
    getHeaders(url) {
        ...
        const cookie = this._mergeCookies(this.browserCookie, this._preferredCookie(), this.loginCookie)
            .split(";")
            .filter((part) => !/^\s*jieqiUserCharset=/i.test(part))
            .join(";")
            .trim();
    ...
    _buildGbkMaps() {
        if (this.gbkDecodeMap) return;
        const decode = {};
        const encode = {};
        for (const chunk of WENKU8_GBK_TABLE) {
            let address = parseInt(chunk[0], 16);
            let previous = -1;
            for (const part of chunk.slice(1)) {
                if (typeof part === "number") {
                    for (let i = 0; i < part; i++) {
                        previous += 1;
                        decode[address++] = previous;
                    }
                    continue;
                }
                for (const character of part) {
                    previous = character.codePointAt(0);
                    decode[address++] = previous;
                }
            }
        }
        for (const address of Object.keys(decode)) {
            const codePoint = decode[address];
            if (encode[codePoint] === undefined) encode[codePoint] = Number(address);
        }
        this.gbkDecodeMap = decode;
        this.gbkEncodeMap = encode;
    }

    _rangeIndex(values, target) {
        let low = 0;
    ...
    _decodeGbk(body) {
        ...
        for (let i = 0; i < body.length; i++) {
            const first = body.charCodeAt(i) & 0xff;
            if (first <= 0x7f) {
                result += String.fromCharCode(first);
                continue;
            }
            const singleByte = this.gbkDecodeMap[first];
            if (singleByte !== undefined) {
                result += String.fromCodePoint(singleByte);
                continue;
            }
            const second = i + 1 < body.length ? body.charCodeAt(i + 1) & 0xff : -1;
            if (first >= 0x81 && first <= 0xfe && second >= 0x30 && second <= 0x39 && i + 3 < body.length) {
                const third = body.charCodeAt(i + 2) & 0xff;
                const fourth = body.charCodeAt(i + 3) & 0xff;
                if (third >= 0x81 && third <= 0xfe && fourth >= 0x30 && fourth <= 0x39) {
                    const pointer = (first - 0x81) * 12600 + (second - 0x30) * 1260
                        + (third - 0x81) * 10 + fourth - 0x30;
                    const range = this._rangeIndex(WENKU8_GB18030_RANGES.gbChars, pointer);
                    const codePoint = WENKU8_GB18030_RANGES.uChars[range]
                        + pointer - WENKU8_GB18030_RANGES.gbChars[range];
                    result += String.fromCodePoint(codePoint);
                    i += 3;
                    continue;
                }
            }
            const codePoint = second >= 0 ? this.gbkDecodeMap[(first << 8) | second] : undefined;
    ...
    _decodeResponse(res) {
        ...
        let containsByte = false;
        for (let i = 0; i < body.length; i++) {
            const code = body.charCodeAt(i);
            if (code > 0xff) return res;
            if (code >= 0x80) containsByte = true;
        }
        if (containsByte) res.body = this._decodeGbk(body);
        return res;
    }
    ...
    _encodeGbk(value) {
        ...
        for (const character of String(value)) {
            ...
            const address = this.gbkEncodeMap[codePoint];
            if (address !== undefined) {
                if (address > 0xff) bytes.push(address >> 8);
                bytes.push(address & 0xff);
                continue;
            }
            const range = this._rangeIndex(WENKU8_GB18030_RANGES.uChars, codePoint);
            let pointer = WENKU8_GB18030_RANGES.gbChars[range]
                + codePoint - WENKU8_GB18030_RANGES.uChars[range];
            bytes.push(
    ...
    async _login() {
        ...
        const body = [
            `username=${this._encodeGbk(username)}`,
            `password=${this._encodeGbk(password)}`,
            "usecookie=315360000",
            "action=login",
            `submit=${this._encodeGbk(" 登 录 ")}`
        ].join("&");
        const headers = {
            ...this.getHeaders(url),
    ...
    _detailUrl(id) {
        return `${this.source.baseUrl}/modules/article/articleinfo.php?id=${id}`;
    }

    _readerUrl(id, chapterId) {
        const cid = chapterId ? `&cid=${chapterId}` : "";
        return `${this.source.baseUrl}/modules/article/reader.php?aid=${id}${cid}`;
    }
    ...
    async search(query, page, filters) {
        const keyword = this._encodeGbk(query.trim());
        const url = `${this.source.baseUrl}/modules/article/search.php?searchtype=articlename&searchkey=${keyword}&page=${page}`;
        const res = await this._request(url, "GET", "", true);
        return this._parseBookList(res.body, page, res.request && res.request.url);
    }
